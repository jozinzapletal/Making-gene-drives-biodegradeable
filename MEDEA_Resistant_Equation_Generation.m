...
T0 = strings(14,14);       %stores the basics combinations and genotype references   
T1 = strings(14,14,10,5);  %stores for each genotype and iteration
...
for i=1:10
    ...
    T0(i+3,1) = strcat('J(', num2str(i+10,'%d'), ')');
end
...
for k=1:10 
  ...
  %determines the gene that is formed otherwise for renaming purposes
                ...
                for b=1:13                              % COMPILES PROBABILITIES
                    for c=1:13
...
if exist('MEDEA Resistant Generated Equations.txt', 'file')==2      
  delete('MEDEA Resistant Generated Equations.txt');
end

% Outputs the equations in the form to be used in the models
diary('MEDEA Resistant Generated Equations.txt');
for i=1:2
    for k=1:10
        if i == 1
            disp(strcat('gprime(', num2str(k,'%d'), ') = (1-fitCost(', num2str(k,'%d'), '))*(1-sigma)*lambda*(', gprime(k), ');'))
        else
            disp(strcat('gprime(', num2str(k+10,'%d'), ') = (1-fitCost(', num2str(k+10,'%d'), '))*(sigma)*lambda*(', gprime(k), ');'))
        end
    end
end
diary off
